        ...
        match action {
            ...
            TeamsPageAction::RemoveUserFromTeam {
                ...
                if FeatureFlag::BillingAndUsagePageV2.is_enabled() {
                    let variant = self.remove_team_member_confirmation_variant(member_email, ctx);
            ...
            TeamsPageAction::RemoveUserFromWorkspace {
                ...
                self.show_team_action_confirmation(
                    ...
                    TeamActionConfirmationTarget::RemoveFromWorkspace {
                        user_uid: *user_uid,
...
impl TeamsPageView {
    ...
    /// Team removal keeps the legacy reload-credits warning; on a native workspace the
    /// member keeps their workspace membership, so the copy says so instead.
    fn remove_team_member_confirmation_variant(
        &self,
        member_email: &str,
        ctx: &ViewContext<Self>,
    ) -> CloudActionConfirmationDialogVariant {
        if let Some(workspace) = self.user_workspaces.as_ref(ctx).current_workspace()
            && workspace.is_native_workspaces_enabled()
        {
            return CloudActionConfirmationDialogVariant::RemoveUserFromNativeWorkspaceTeam {
    ...
    fn team_to_item_list(
        ...
        team.members.iter().for_each(|member| {
            ...
            if member.email != *current_user_email {
                ...
                // Workspace admins can remove anyone but the workspace owner from the
                // whole workspace, whatever their team role.
